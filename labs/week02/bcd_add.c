...
int maximum(big_bcd_t *x->n_bcd, big_bcd_t *y->n_bcd);
...
big_bcd_t *bcd_add(big_bcd_t *x, big_bcd_t *y) {
    // Pointer to Array to store big_bcd for x + y
    big_bcd_t *s = (big_bcd_t *)malloc(sizeof(big_bcd_t));
    // Actual array for the string x + y
    int max = maximum(x->n_bcd, y->n_bcd);
    s->bcd = (unsigned char *)malloc((max*sizeof(char))+2);
    
    int c = 0; //Carry
    int sum;
    for (int i = 0; i < max; i++){
        if((i < x->n_bcd) && (i < y->n_bcd)){
            sum = x->bcd[i] + y->bcd[i] + c;
            if(sum >= 10){
                s->bcd[i] = sum - 10;
                c = 1;
            } else {

...
main:
    ...
    li $s1, 4           # set sizeof(int) = 4 
    li $s2, 0           # int i = 0;       

loopx:
    beq $s2, $t0, print  # if (i >= x) break 
    li $s3, 0           # j = 0;
    mul $t3, $s2, 4     # off = 4*i  
    #lw $s4, array($t3)  # yp = &matrix[i][0]
loopy:
    beq $s3, $t1, end2  # if (j >= y) break 

    mul $t4, $s3, 4     # off = 4*j

    #add $t3, $t3, $s4   # int *p = &rowp[x]
    lw $t3, array($t3)  # t3 = array[x][y]
